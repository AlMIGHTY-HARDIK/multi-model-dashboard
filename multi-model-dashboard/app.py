...
##############################################################################
# Helper: Decide SHAP Explainer Type Based on Model Key
##############################################################################
...
##############################################################################
# Helper: Create a Precision-Recall Plot (returns base64-encoded PNG)
##############################################################################
        ...
        components = [
            dbc.Row([
                dbc.Col(
                    ...
                    ], className="shadow-sm", style={'borderRadius': '15px', 'border': '1px solid #007bff', 'padding': '10px'}),
                    width=6
                ),
                dbc.Col(
                    dbc.Card([
                    ...
                    ], className="shadow-sm", style={'borderRadius': '15px', 'border': '1px solid #28a745', 'padding': '10px'}),
                    width=6
                )
            ]),
            dbc.Row([
...
def load_dataset(dataset_key):
    mapping = {
        "heart":  r"multi-model-dashboard\\saved_datasets\\Heart_UCI.csv",
        "house":  r"multi-model-dashboard\\saved_datasets\\House_Price_Prediction.csv",
        "loan":   r"multi-model-dashboard\\saved_datasets\\Loan_Prediction.csv",
        "stocks": r"multi-model-dashboard\\saved_datasets\\Stocks.csv",
        "adult":  r"multi-model-dashboard\\saved_datasets\\UCI_Adult_Census_Income.csv"
    }
...
# The header container with the dashboard heading
header = dbc.Container([
    dbc.Row(
        dbc.Col(
            html.Div([
                html.H1("Multi Model Explainer Dashboard", className="display-4"),
                html.P(
                    "This dashboard integrates various machine learning models and provides "
                    "detailed explainability insights. Explore performance metrics, model predictions, "
                    "SHAP visualizations, and bias checks through interactive charts and components. "
                    "Each section is animated for a smooth user experience.",
                    className="lead"
                )
    ...
    html.Hr()
], fluid=True, style={'marginBottom': '70px'})
...
# Include the 'header' as the first child in the layout
app.layout = dbc.Container(fluid=True, children=[
    header,  # <--- THIS is what was missing
    dbc.Row([
        dbc.Col([
...
def update_interface(selected_dataset, selected_model):
    ...
    # Return the dummy bar charts and the custom dashboard layout
